...
document.addEventListener("DOMContentLoaded", function () {
    ...
    const storyTemplates = [
      "It was a [adjective] [time] day, and [name] decided to go for a walk. ",
      "Once upon a time, in a [adjective] kingdom, there lived [name]. ",
      "In a land far, far away, [name] woke up on a [adjective] morning. ",
      "[name] looked out of the window on a [adjective] day and thought, 'I need an adventure.' ",
    ];
  
    const adjectives = ["sunny", "cloudy", "rainy", "magical", "wonderful"];
    const times = ["bright", "gloomy", "windy", "stormy", "peaceful"];
    ...
    randomizeButton.addEventListener("click", function () {
      const randomStoryTemplate = getRandomItem(storyTemplates);
      ...
      if (imperialRadio.checked) {
        temperature = `${Math.floor(Math.random() * 100)}°F`;
      } else {
        temperature = `${Math.floor(Math.random() * 40)}°C`;
      }
  
      const story = randomStoryTemplate
